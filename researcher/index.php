...
// Logged-in researcher ID
$login_id = $_SESSION['id'];

// Fetch all approved species (feed)
$feed_q = mysqli_query(
  $con,
  "SELECT 
    species.id,
...
<style>
  /* Uniform thumbnail size without changing layout */
  body { background:#E9F2EC; }

  .species-img {
    width:100%;
    height:200px;             /* fixed card image height */
    object-fit:cover;          /* crop/fit neatly */
    border-top-left-radius:12px;
    border-top-right-radius:12px;
  }

  /* Keep card heights consistent if text is long */
  ...
  /* Make "See more" expand inline */
  .card-text .collapse.show { display:inline; }
</style>
...
<div class="container py-4" style="background:#E9F2EC;">
  ...
  <!-- Page Heading -->
  <div class="page-head">
    <h1 class="page-title">Researcher Home</h1>
    <p class="page-sub">Upload sightings, track approvals, and engage with others</p>
  </div>

  <!-- Main action buttons -->
  <div class="d-flex gap-2 flex-wrap mb-4">
    <a href="verifyspecies.php" class="btn btn-success rounded-pill"
       style="background:#2EB872; border-color:#2EB872;">
      Verify Species
    </a>
    <a href="researcher_dashboard.php" class="btn btn-outline-success rounded-pill"
       style="border-color:#2EB872; color:#2EB872;">
      Dashboard
    </a>
  </div>
  ...
  <div class="row g-4">
    ...
    <?php if($feed_q && mysqli_num_rows($feed_q) > 0){ ?>
      ...
      // Which species current user liked
      $user_likes = [];
      $ul_res = mysqli_query(
        $con,
        "SELECT species_id FROM species_likes WHERE login_id='$login_id'"
      );
      if($ul_res){
      ...
      <?php while($row = mysqli_fetch_assoc($feed_q)){
        ...
        // For "See more" toggle in description
        $desc_id  = 'desc-' . $sid;
      ...
      <div class="col-md-4 col-sm-6">
        <div class="card h-100 border-0 shadow-sm species-card">
          ...
          <div class="card-body">
            ...
            <!-- Short description with optional "See more" -->
            <p class="card-text" style="color:#5B6B61;">
          ...
          <div class="card-footer bg-white border-0 d-flex justify-content-between align-items-center">
            ...
            <button class="btn btn-sm btn-outline-secondary comment-btn"
              ...
              Comment
              <span class="comment-count"><?= count($comments[$sid] ?? []) ?></span>
            </button>
          </div>
      ...
      <div class="modal fade" id="modal<?= $sid ?>" tabindex="-1" aria-hidden="true"
           aria-labelledby="modalLabel<?= $sid ?>">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <p><strong>Contributor:</strong> <?= htmlspecialchars($row['contributor_name'] ?? 'Unknown'); ?></p>
