...
<div class="container py-5" style="background:#E9F2EC;">
  <div class="page-head container">
    <div class="row align-items-center g-3">
      <div class="col">
        <h1 class="page-title m-0">Researchers</h1>
        <p class="page-sub m-0">Directory of registered researchers</p>
      </div>
      <div class="col-auto">
        <a href="addresearcher.php" class="btn btn-success btn-sm rounded-pill">Add Researcher</a>
      </div>
    </div>
  </div>

  <?php
  $str = "SELECT researcher.*, login.user_status, login.id AS login_id
          FROM researcher
    ...
    <table class="table table-bordered table-striped table-hover align-middle text-center">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            <!-- <a href="edit_researcher.php?id=<?= $data['login_id']; ?>" class="btn btn-success btn-sm mb-1">Edit</a> -->
            <a href="delete_researcher.php?id=<?= $data['login_id']; ?>" class="btn btn-danger btn-sm mb-1"
               onclick="return confirm('Delete this researcher?');">Delete</a>

            <?php if ($data['user_status'] == 'pending') { ?>
              <a href="approved.php?id=<?= $data['login_id']; ?>" class="btn btn-success btn-sm mb-1">Verify</a>
              <a href="reject.php?id=<?= $data['login_id']; ?>" class="btn btn-danger btn-sm mb-1">Revoke</a>
            <?php } elseif ($data['user_status'] == 'approved') { ?>
              <a href="reject.php?id=<?= $data['login_id']; ?>" class="btn btn-danger btn-sm mb-1">Revoke</a>
  ...
  <?php
    ...
    echo "<h4 class='text-center text-muted'>No researchers found.</h4>";
  }
